...
bool PlayLayer::init()
{
    ...
    this->schedule(schedule_selector(PlayLayer::checkGrid, ), 0.1);
    this->schedule(schedule_selector(PlayLayer::checkWin, ), 0.1);
...
void PlayLayer::displayOWin(cocos2d::Vec2 a, cocos2d::Vec2 b, cocos2d::Vec2 c)
{
...
void PlayLayer::displayTitleWin(int player)
{
    if (player == 1)
    {
...
void PlayLayer::displayXWin(cocos2d::Vec2 a, cocos2d::Vec2 b, cocos2d::Vec2 c)
{
...
void PlayLayer::checkWin(float dt)
{
    // kiểm tra thời gian
...
void PlayLayer::checkGrid(float dt)
{
    for (int i = 0; i < 3; i++)
    {
        for (int j = 0; j < 3; j++)
        {
            ...
            std::string imageXName = "X" + std::to_string(i) + std::to_string(j);
                ...
                button->addClickEventListener([=](Ref*)
                    {
                        ...
                        else if(m_enableOChecked == false)
                        {
                            if (m_MatrixChecked[i][j] == 0)
                            {
                                this->removeChildByName("progressTimer");
...
bool PlayLayer::isOChecked()
{
    return m_enableOChecked;
}

void PlayLayer::setOChecked(bool x)
{
    m_enableOChecked = x;
...
void PlayLayer::setMatrixChecked(char x, char y)
{
    if (isOChecked())
    {
...
void PlayLayer::initMaTrixChecked()
{
    for (int i = 0; i < 3; i++)
    {
        for (int j = 0; j < 3; j++)
...
bool PlayLayer::isEmptyGridComp(char x, char y)
{
...
void PlayLayer::showProgressTimer()
{ 
    auto visibleSize = Director::getInstance()->getVisibleSize();
