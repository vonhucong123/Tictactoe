...
void AI::PlacePiece(int(*gridArray)[3][3])
{
    try
    {
        ...
        {
            CheckSection(checkMatchVector[i][0], checkMatchVector[i][1], checkMatchVector[i][2], checkMatchVector[i][3], checkMatchVector[i][4], checkMatchVector[i][5], 2, gridArray);
        }
        ...
        CheckIfPieceIsEmpty(1, 1, gridArray);

        // check if a corner is empty
        CheckIfPieceIsEmpty(0, 2, gridArray);
        CheckIfPieceIsEmpty(2, 2, gridArray);
        CheckIfPieceIsEmpty(0, 0, gridArray);
        CheckIfPieceIsEmpty(2, 0, gridArray);

        // check for any other empty piece
        CheckIfPieceIsEmpty(1, 2, gridArray);
        CheckIfPieceIsEmpty(0, 1, gridArray);
        CheckIfPieceIsEmpty(2, 1, gridArray);
        CheckIfPieceIsEmpty(1, 0, gridArray);
    }
    catch (int error)
    {
...
void AI::CheckSection(int x1, int y1, int x2, int y2, int X, int Y, int pieceToCheck, int(*gridArray)[3][3])
{
